...
const QuestionPaperModal = () => {
  ...
  const round = useSelector((state) => state.roundTab.value);
  useEffect(() => {
    const sectionRequest = getSectionList();
    sectionRequest(dispatch, round);
  }, []);
                        ...
                        <Box sx={{ display: "flex", alignContent: "center" }}>
                          ...
                          <AvatarGroup max={4}>
                            {question.asignee.map((asignee, id) => {
                              return <Avatar {...stringAvatar(asignee.name)} />;
                            })}
                          </AvatarGroup>
                        </Box>
                        <Box>
